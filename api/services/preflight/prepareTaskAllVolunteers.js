    ...
    userUtils.findPrimaryEmail(fields.recipientId, function (err, userEmail) {
      ...
      Task.findOneById(fields.callerId).exec(function (err, task) {
        ...
        Volunteer.find({ taskId: task.id }).exec(function(err, vols) {
          var users = vols.map(function(vol) { return vol.userId; });
          async.map(users, userUtils.findPrimaryEmail, function (err, emails) {
            if (err) { sails.log.debug(err); cb(null, content); return false; }
            if (emails) content.fields.to = emails.map(function(user) {
              return user.email;
            });
            next();
          });
